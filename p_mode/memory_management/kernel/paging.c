...
uint32_t alloc_page(uint32_t *directory, uint32_t vaddr, uint32_t is_writeable, uint32_t is_kernel) {
	return alloc_page_pr(directory, vaddr, first_free_frame(), is_writeable, is_kernel, 0, directory != kernel_directory);
}
...
void add_frame_to_kernel_dir(uint32_t frame_addr) {
	printf("adding frame %x to kernel directory (%x)\n", frame_addr, kernel_directory);
...
uint32_t alloc_page_pr(uint32_t *directory, uint32_t vaddr, uint32_t phys_addr, uint32_t is_writeable, uint32_t is_kernel, uint32_t is_privileged, uint8_t demand_paging) {
	...
	if((directory[tableIndex] & 0x1) == 0) {
		...
		directory[tableIndex] = (uint32_t)table_addr | ((is_writeable?2:0) | (is_kernel?0:4) | !demand_paging);
		memset((void*)(directory[tableIndex] & 0xFFFFF000), 0, PAGES_PER_TABLE * sizeof(uint32_t));
	}
	uint32_t table_entry = demand_paging ? 0x1000000 : phys_addr;
	directory[tableIndex] |= ((is_writeable)?2:0) + ((is_kernel)?0:4);
	table_entry |= ((is_writeable)?2:0) + ((is_kernel)?0:4) + !demand_paging;
	...
	if(!demand_paging) write_bitset(phys_addr, 1);
...
void page_fault_handler(regs_t r) {
	...
	uint32_t* directory = (uint32_t*) read_cr3();
	uint32_t page = get_page(current_dir, cr2);
	
	printf("\nFaulty Directory: %x", directory);
	...
	printf("\nPage for that address: %x\n", page);
	printf("Table for that address: %x\n", current_dir[TABLE_INDEX_FROM_VIRTUAL_ADDRESS(cr2)]);
	print_regs(r);
	
	if(directory == kernel_directory || ((errcode&1) == 0 && ((page&0xFFFFF000) != 0x1000000))) {
		printf("\nSystem Halted!");
		for(;;);
	}
	
	uint32_t tableIndex = TABLE_INDEX_FROM_VIRTUAL_ADDRESS(cr2);
	uint32_t pageIndex = PAGE_INDEX_FROM_VIRTUAL_ADDRESS(cr2);
	
	uint32_t* table_ptr = (uint32_t*) (directory[tableIndex] & 0xFFFFF000); // removing flags for getting actual address of table
	uint32_t table_entry = table_ptr[pageIndex] & 0xFFF;
	uint32_t phys_addr = first_free_frame();
	table_entry |= phys_addr;
	table_ptr[pageIndex] = table_entry + 1;
	
	write_bitset(phys_addr, 1);
	add_frame_to_kernel_dir(phys_addr);	
}
